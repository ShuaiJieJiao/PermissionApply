...
class ProxyInfo(var element: Element, processingEnv: ProcessingEnvironment) {
    ...
    // 添加导包
    fun addImport(importStr: String) {
        importList.add(importStr)
    }

    // 添加允许方法
    fun addAllow(requestCode: Int, methodName: String) {
        allow.put(requestCode, methodName)
    }

    // 添加拒绝方法
    fun addRefuse(requestCode: Int, methodName: String) {
        refuse.put(requestCode, methodName)
    }

    // 添加解释方法
    fun addExplanation(requestCode: Int, methodName: String) {
        explanation.put(requestCode, methodName)
    }
